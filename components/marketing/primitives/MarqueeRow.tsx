...
export function MarqueeRow({
  ...
  if (reduce) {
    return <div className={cn('flex flex-wrap items-center justify-center gap-x-12 gap-y-4', className)}>{children}</div>;
  }
  ...
  return (
    <div className={cn('relative overflow-hidden', className)}>
      ...
      <div className={cn('flex w-max [animation:marquee_linear_infinite] hover:[animation-play-state:paused]', durationClassName)}>
        {items}
        {items}
      </div>
    </div>
