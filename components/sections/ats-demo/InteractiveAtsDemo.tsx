...
import {
  ArrowRight, FileText, Loader2, Lock, CheckCircle2,
  ScanLine, TrendingUp, Briefcase, BadgeCheck, RotateCcw,
} from 'lucide-react';
...
function MetricRow({
  icon: Icon, label, value, prog, text,
}: {
...
export function InteractiveAtsDemo() {
  ...
  React.useEffect(() => {
    if (reduce) { setGauge(phase === 'idle' ? 64 : result.score); return; }
    let raf = 0;
    if (phase === 'idle') {
  ...
  React.useEffect(() => {
    ...
    return () => { clearInterval(stepTimer); clearTimeout(done); };
  }, [phase]);
  ...
  const prog = Math.max(0, Math.min(1,
    phase === 'idle' ? gauge / 100
      : phase === 'analyzing' ? gauge / Math.max(1, result.score)
      : 1,
  ));
      ...
      <div className="mx-auto max-w-6xl">
        <Reveal from="up" distance={20}>
          <SectionHeading
            ...
            title={<>Watch an ATS read your résumé in <span className="text-gradient">real time</span></>}
          ...
          <GlassCard glow className="relative mt-12 overflow-hidden p-5 sm:p-8">
              ...
              <div className="relative">
                <div className="mb-3 flex items-center justify-center gap-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground lg:justify-start">
                  <span className="relative inline-flex h-2 w-2">
                    {phase === 'analyzing' && <span className="absolute inset-0 animate-ping rounded-full bg-primary opacity-75" />}
                    <span className="relative inline-flex h-2 w-2 rounded-full bg-primary" />
                  </span>
                  {phase === 'analyzing' ? 'Scoring…' : phase === 'teaser' ? 'Preview ready' : 'Live ATS score'}
                </div>

                <div className={cn('transition-[filter] duration-500', gated && 'pointer-events-none select-none blur-[7px]')} aria-hidden={gated}>
                  <div className="flex flex-col items-center gap-7 lg:flex-row lg:items-center lg:gap-8">
                    ...
                    <div className="w-full max-w-xs space-y-3.5">
                      {result.metrics.map((m) => (
                        <MetricRow key={m.key} icon={m.icon} label={m.label} value={m.value} prog={prog} />
                      ))}
                      <MetricRow
                        icon={BadgeCheck}
                  ...
                  {gated && (
                    ...
                    >
                      ...
                      <p className="max-w-xs text-sm text-muted-foreground">
                        Sign up free to reveal your real ATS score, the keywords you&apos;re missing, and exactly how to fix them.
                      </p>
            ...
            {phase === 'idle' && (
              <div className="mt-7 flex flex-col items-center justify-center gap-3 border-t border-border/50 pt-6 sm:flex-row">
                <p className="text-sm text-muted-foreground">This is a live demo — drop your résumé to scan yours.</p>
                <ButtonLink href={appUrl('/signup')} variant="glow" size="sm" className="group">
                  Create free account
                  <ArrowRight className="transition-transform group-hover:translate-x-0.5" />
                </ButtonLink>
            ...
            {phase === 'teaser' && (
              <div className="mt-7 flex items-center justify-center gap-2 border-t border-border/50 pt-6 text-xs text-success">
                <CheckCircle2 className="h-4 w-4" /> Scan complete — sign up to see the full breakdown.
              </div>
            )}
          </GlassCard>
        </Reveal>
